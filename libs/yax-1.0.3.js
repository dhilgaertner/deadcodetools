...
(function($) {
    ...
    function RTCPeerConnection(options) {
        ...
        optional.mandatory = { 'googIPv6' : false };
...
function Talk(apiKey, readyCallback, eventCallback) {
    var self = this;
    var _apiKey = apiKey;
    var callerIdNumber = "1111";
    var bridgeDomain = "bridge.yax.io";
    var activeCall;
    var callbacks = {
        onMessage: function(verto, dialog, msg, data) {
            console.debug("onMessage: " + data.eventType);

            if (data.eventType == "channelPvtData" && data.pvtData !== null && data.pvtData.action == "conference-liveArray-join") {
                self.verto.subscribe(data.pvtData.laChannel, {
                    handler: function(v, e) {
                        var audioData = JSON.parse(e.data.data[4]);
                        var participant = {
                            "action": e.data.action,
                            "name" : e.data.data[2],
                            "muted" : audioData.audio.muted,
                            "energy" : audioData.audio.energyScore
                        };
                        if (__isFunction(eventCallback)) eventCallback("MODIFIED", participant);
                    }
                });
            }
        },
        ...
        onDialogState: function(d) {
            console.debug("onDialogState: " + d.state.name);

            switch (d.state.name) {
                case "trying":
                    if (__isFunction(eventCallback)) eventCallback("CONNECTING");
                    break;
                case "active":
                case "recovering":
                    if (d.params.destination_number !== undefined && d.params.destination_number.indexOf("room_") > -1) {
                        self.conferenceResource = d.params.destination_number + "-" + bridgeDomain;
                    } else if (d.params.remote_caller_id_number !== undefined && d.params.remote_caller_id_number.indexOf("room_") > -1) {
                        self.conferenceResource = d.params.remote_caller_id_number + "-" + bridgeDomain;
                    }

                    if (__isFunction(eventCallback)) eventCallback("CONNECTED");
                    break;
                case "hangup":
                    if (__isFunction(eventCallback)) eventCallback("DISCONNECTING");
                    break;
                case "destroy":
                    if (__isFunction(eventCallback)) eventCallback("DISCONNECTED");
                    break;
                default:
                    console.log("unhandled state: " + d.state.name);
                    break;
            }
        },

        onWSLogin: function(v, success) {
            console.debug("onWSLogin: " + success);
            if (success) {
                if (__isFunction(readyCallback)) readyCallback();
                if (__isFunction(eventCallback)) eventCallback("READY");
            }
        },

        onWSClose: function(v, success) {
            console.debug("onWSClose: " + v);
        },
    ...
    this.connect = function(room, displayName, callback) {
        activeCall = this.verto.newCall({
            destination_number: "room_" + _apiKey + "_" + room,
    ...
    this.mute = function() {
        __sendConferenceCommand("mute");
    };

    this.unmute = function() {
        __sendConferenceCommand("unmute");
    };

    this.getRoom = function() {
        return talk.verto.dialogs[__dialog().callID].params.destination_number;
    };

    __dialog = function() {
        var dialog = null;

        $.each(self.verto.dialogs, function(index, element) {
            dialog = element;
        });
    ...
    __sendConferenceCommand = function(command, argument) {
        __getVariable(this.__dialog().callID, "conference_member_id", function(value) {
            __sendCommand("conference", self.conferenceResource + " " + command + " " + value.message);
        }, function(e) {
            // error handling
        });
    };

    __getVariable = function(uuid, variable, success, error) {
        var payload = {
            'eventChannel' : 'conference',
            'cmd' : 'uuid_getvar',
            'arg' : uuid + ' ' + variable
        };
        console.log("sendingCommand:: " + JSON.stringify(payload));
        self.verto.rpcClient.call('fsapi', payload,
            success,
            error
        );
    };

    __sendCommand = function(command, argument) {
        var payload = {
            'eventChannel' : 'conference',
            'cmd' : command,
            'arg' : argument
        ...
        console.log("sendingCommand:: " + JSON.stringify(payload));
        __dialog().verto.sendMethod('fsapi', payload);
    };

    __isFunction = function(functionToCheck) {
        var getType = {};
    ...
    $(function() {
        if (DetectRTC.isWebRTCSupported) {
            $("body").prepend("<video width=800 id='yaxObject' autoplay='autoplay' hidden='true'></video>");

            self.verto = new $.verto({
                login: _apiKey + "@bridge.yax.io",
                passwd: "1b4f0e9851971998e732078544c96b36c3d01cedf7caa332359d6f1d83567014",
                socketUrl: "wss://app.yax.io:8082",
                tag: "yaxObject",
            ...
            $("body").prepend("yax not supported, by this browser");
        }
    });
}

(function(){
    LE.init('24b6f96f-6404-47b3-a728-f88cdf248de2');
    var browserConsoleLogEnabled = true;

    if (window.console && console.log) {
        var oldLog = console.log;
        console.log = function() {
            LE.log(arguments);
            if (browserConsoleLogEnabled) oldLog.apply(this, arguments);
        };
    }
    ...
    if (window.console && console.debug) {
        ...
        console.debug = function() {
            LE.log(arguments);
            if (browserConsoleLogEnabled) oldDebug.apply(this, arguments);
        };
    ...
    if (window.console && console.error) {
        ...
        console.error = function() {
            LE.log(arguments);
            if (browserConsoleLogEnabled) oldError.apply(this, arguments);
        };
    }
    ...
    if (window.console && console.info) {
        ...
        console.info = function() {
            LE.log(arguments);
            if (browserConsoleLogEnabled) oldInfo.apply(this, arguments);
        };
    }
    ...
    if (window.console && console.trace) {
        ...
        console.trace = function() {
            LE.log(arguments);
            if (browserConsoleLogEnabled) oldTrace.apply(this, arguments);
        };
    }
    ...
    if (window.console && console.warn) {
        ...
        console.warn = function() {
            LE.log(arguments);
            if (browserConsoleLogEnabled) oldWarn.apply(this, arguments);
        };
    }
})();